...
class AuthorizationControllerTest extends TestCase
{
    ...
    {
        RaazPuspa\Passport\Passport::tokensCan([
            'scope-1' => 'description',
        ]);
        ...
        $controller = new RaazPuspa\Passport\Http\Controllers\AuthorizationController($server, $response);
        ...
        $authRequest->shouldReceive('getScopes')->andReturn([new RaazPuspa\Passport\Bridge\Scope('scope-1')]);
    ...
    public function test_request_is_approved_if_valid_token_exists()
    {
        ...
        $authRequest->shouldReceive('getScopes')->once()->andReturn([new RaazPuspa\Passport\Bridge\Scope('scope-1')]);
        ...
        $tokens = Mockery::mock('RaazPuspa\Passport\TokenRepository');
        $tokens->shouldReceive('findValidToken')->with($user, 'client')->andReturn($token = Mockery::mock('RaazPuspa\Passport\Token'));
